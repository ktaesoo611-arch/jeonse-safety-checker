...
function parseKoreanAddress(address: string): {
  ...
  const sidoMatch = address.match(/^(서울특별시|부산광역시|대구광역시|인천광역시|광주광역시|대전광역시|울산광역시|세종특별자치시|경기도|강원특별자치도|강원도|충청북도|충청남도|전북특별자치도|전라북도|전라남도|경상북도|경상남도|제주특별자치도)/);
  if (sidoMatch) {
    result.sido = sidoMatch[1];
  }

  // Extract 시군구 (follows sido, ends with 구, 시, 군)
  const sigunguMatch = address.match(/(?:특별시|광역시|특별자치시|특별자치도|도)\s+([가-힣]+(?:구|시|군))/);
  if (sigunguMatch) {
